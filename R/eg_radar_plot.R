...
eg_radar_plot <- function(plot.data,
  ...
  plot.data[,grouping.var] <- as.factor(as.character(plot.data[,grouping.var]))
  ...
  base <- base + geom_path(data=group$path,aes(x=x,y=y,group=group,colour=group),
                           size=group.line.width, alpha=group.line.alpha)

  # ... + group points (cluster data)
  base <- base + geom_point(data=group$path,aes(x=x,y=y,group=group,colour=group),size = group.point.size, alpha=group.point.alpha)
